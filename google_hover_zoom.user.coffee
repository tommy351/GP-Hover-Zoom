...
init =
	basic: ->
		...
		$set.on('click', '.close, .back', setPage.close)
		# Save options
		...
		# Tab events
		.find('.menu li').each (i) ->
			$(this).addClass('current') if i is 0
			$(this).attr('tabid', i).on 'click', ->
				setPage.tab(i)
	history: ->
...
<div class='hz_button green save' title='#{lang.set02}'>#{lang.set02}</div>
<div class='hz_button white reset' title='#{lang.set03}'>#{lang.set03}</div>
<a class='hz_button blue update' href='http://userscripts.org/scripts/source/106681.user.js' title='#{lang.update01}'>#{lang.update01}</a>
</div>
</div>"
			menu:
				setting:
...
$(document).ready ->
	...
	update.auto() if options.hz_update is 'true' and !developer
...
update = new ->
	...
	$meta.html(lang.update09) if developer
	...
	fetch = ->
		GM_xmlhttpRequest
			...
			onload: (data) ->
				...
				content = data.content[options.hz_language] or data.content['en-US']

				if version is data.version
					$meta.html(lang.update07).addClass('green')
				else
					length = if nowVer.length > newVer.length then nowVer.length else newVer.length
					i = 0;
					while i < length
						nowVer[i] = 0 if typeof nowVer[i] is 'undefined'
						newVer[i] = 0 if typeof newVer[i] is 'undefined'

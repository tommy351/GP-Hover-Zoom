...
<div class='main'>
	...
	<div class='functions top'>
		...
		<div class='hz_button blue' title='#{lang.al05}'>#{lang.al05}</div>
		<div class='hz_button green' title='#{lang.al06}'>#{lang.al06}</div>
		<div class='hz_button orange' title='#{lang.al04}'>#{lang.al03}</div>
	</div>
</div>"
...
sortPic = (obj, fragment) ->
	...
	autoLoad = ->
		if $(this).scrollTop() >= this.scrollHeight - $(this).height() - 200 and !trigger
			start = 50 * counts
...
albumDL = ->
	...
	GM_xmlhttpRequest
		...
		onload: (data) ->
			...
			$page.find('small').html("<a href='#{feed.author[0].uri.$t}' target='_blank'>#{feed.author[0].name.$t}</a> &raquo; <a href='#{feed.link[3].href}' target='_blank'><strong>#{feed.title.$t}</strong></a> (#{entry.length} #{if entry.length > 1 then lang.set08 else lang.set07})")
			sortPic($page, fragment)
...
maxYT = ->
	$content.on 'click', 'div[data-content-type$="flash"]', ->
		...
		$(this).attr('style', 'max-height: none; max-width: none; width: 100;').load ->
			iframe = $(this)
			button = $('<span>').addClass('c-C').attr('title', lang.set10).html(" - #{lang.set10}").click ->
				iframe.remove().nextAll().show().parent().attr('style', 'height: 226px;').click ->
...
$(document).ready ->
	...
	maxYT() if options.maxyt is 'true'

...
;
var $content, albumDL, batch, bodyElement, bodyTmp, disable, elements, enable, execHash, gcRegex, history, hoverzoom, init, lang, locale, maxYT, menuElements, menuTmp, mouse, options, picRegex, picasaRegex, sortPic, timer, update, version, ytDL;
...
sortPic = function(obj, fragment) {
  var $inner, $wrap, addFragment, autoLoad, copyLink, counts, length, max, newTab, trigger, wHeight, wWidth;
  wWidth = obj.width();
  wHeight = obj.height();
  $wrap = obj.find('.wrap');
  ...
  counts = 0;
  trigger = false;
  addFragment = function(start, end) {
    var appends, i;
  ...
  autoLoad = function() {
    var end, start;
    if ($(this).scrollTop() >= this.scrollHeight - $(this).height() - 200 && !trigger) {
      start = 50 * counts;
      ...
      return addFragment(start, end);
    }
  };
  copyLink = function() {
    var $page, $textarea, appends, i, url, _i, _len;
    $page = $('#hz_copyarea');
  ...
  newTab = function() {
    ...
    for (_i = 0, _len = fragment.length; _i < _len; _i++) {
      i = fragment[_i];
      _results.push(window.open(i.href, 'hz_newtab' + i));
    }
    return _results;
  };
  if (length > 0) {
  ...
  return obj.fadeIn(300).on('click', '.blue', copyLink).on('click', '.green', newTab).children('.main').css({
    width: wWidth - 200,
    height: wHeight - 140,
...
albumDL = function() {
  var $page, albumid, data, fragment, userid, width;
  $page = $('#hz_album_page');
  data = $(this).data('url');
  ...
  width = parseInt(($page.width() - 200) / options.hz_his_columns - 10);
  fragment = [];
  $page.fadeIn(300).find('small').html(lang.fs04).end().find('.orange').attr('href', "picasa://downloadfeed/?url=https://picasaweb.google.com/data/feed/back_compat/user/" + userid + "/albumid/" + albumid + "?imgdl=1");
  return GM_xmlhttpRequest({
    ...
    onload: function(data) {
      ...
      $(entry).each(function(i, data) {
        var a, item, original, thumbnail, url;
        a = data.media$group;
        url = a.media$content[0].url;
        original = url.replace(/(.*)\//, '$1/s0/');
        thumbnail = url.replace(/(.*)\//, "$1/w" + width + "/");
        item = document.createElement('a');
        $(item).html("<img src='" + thumbnail + "' width='" + width + "'>").attr({
          href: original,
          title: a.media$title.$t
        });
        return fragment.push(item);
      });
      $page.find('small').html("<a href='" + feed.author[0].uri.$t + "' target='_blank'>" + feed.author[0].name.$t + "</a> &raquo; <a href='" + feed.link[3].href + "' target='_blank'><strong>" + feed.title.$t + "</strong></a> (" + entry.length + " " + (entry.length > 1 ? lang.set08 : lang.set07) + ")");
      return sortPic($page, fragment);
    }
  });
};
...
batch = function() {
  ...
  return $page.find('small').html("<strong>" + count + "</strong> / " + options.hz_his_max + (count > 1 ? lang.set08 : lang.set07));
};
...
maxYT = function() {
  return $content.on('click', 'div[data-content-type$="flash"]', function() {
    ...
    return $(this).attr('style', 'max-height: none; max-width: none; width: 100;').load(function() {
      var button, iframe;
      iframe = $(this);
      button = $('<span>').addClass('c-C').attr('title', lang.set10).html(" - " + lang.set10).click(function() {
        iframe.remove().nextAll().show().parent().attr('style', 'height: 226px;').click(function() {
          $(this).attr('style', 'max-height: none; max-width: none; width: 100;').prepend(iframe).children().hide();
          button.show();
          return false;
        });
        return $(this).hide();
...
ytDL = function(url, ele) {
  ...
  return GM_xmlhttpRequest({
    ...
    url: url,
    onerror: function() {
      return ele.children('.notify').html(lang.ytdl07);
    },
    onload: function(data) {
      var i, map, regexp_url, title, _fn, _i, _len;
      ...
      if (map.length > 0) {
        _fn = function(i) {
          ...
          if (item.url != null) {
            ...
            return appends += _i === 0 ? "<a class='c-C' href='" + url + "' target='_blank'>" + desc + "</a>" : "<br><a class='c-C' href='" + url + "' target='_blank'>" + desc + "</a>";
          }
        };
        ...
        return ele.children('.notify').html(lang.ytdl08);
      }
    }
  });
};
  ...
  tube = function() {
    return $('div[data-content-type$="flash"]').each(function() {
      ...
      if (!$(this).data('class')) {
        ...
        button = $("<span class='c-C tubeStacks'>" + lang.fs03 + "</span>").click(function() {
          var popInner, popup;
        ...
        if (children.length > 0) {
          ...
          zoom = $("<span class='c-C' title='" + lang.maxpic01 + "'> - " + lang.maxpic01 + "</span>").click(function() {
            children.each(function() {
              ...
              if (data != null) {
                return $(this).attr('style', '').children('img').attr('src', data);
              }
            });
...
update = function(manual) {
  var main;
  main = function(news, content, latest) {
    var $page, appends, i, _i, _len;
    $page = $('#hz_update_note');
    appends = '';
    if (latest) {
      $page.find('.green').hide().end().find('small').html("<strong>" + version + "</strong>" + lang.update06);
    } else {
      $page.find('.green').show().end().find('small').html("" + lang.update02 + "<strong>" + news + "</strong> / " + lang.update06 + "<strong>" + version + "</strong>");
    }
    for (_i = 0, _len = content.length; _i < _len; _i++) {
      i = content[_i];
      appends += "<li>" + i + "</li>";
    }
    return $page.fadeIn(300).find('p').html("<ul>" + appends + "</ul>");
  };
  return GM_xmlhttpRequest({
    method: 'GET',
    url: 'https://sites.google.com/site/hoverzoomplus/sources/version.json',
    onload: function(data) {
      var content, feed, i, newVer, nowVer, _ref, _ref2, _results;
      ...
      if (version === feed.version && manual) {
        return main(version, content, true);
      } else {
        _results = [];
        for (i = _ref = nowVer.length, _ref2 = newVer.length; _ref <= _ref2 ? i <= _ref2 : i >= _ref2; _ref <= _ref2 ? i++ : i--) {
          if (nowVer[i] != null) nowVer[i] = 0;
          if (newVer[i] > nowVer[i]) {
            main(feed.version, content, false);
            break;
          } else if (nowVer[i] > newVer[i]) {
            if (manual) main(version, content, true);
            break;
          } else {

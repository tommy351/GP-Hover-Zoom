	...
	var maxPic = function(){
		$('.B9JpJ').filter(':visible').each(function(){
			if (!$(this).data('class')){
				...
				if (options.hz_maxpic_option === '1' || children.length > 3) children = children.eq(0);

				children.each(function(){
					$(this).attr('src', this.src.replace(picasaRegex, '/w'+parentWidth+'/$2')).parent().parent().addClass('hz_maxPic_container');
				});

...
var hoverzoom = function(){
	...
	// General Tab
	set.innerHTML += '<div class="hz_set_tab">'+
	...
	'<input id="hz_resolution" type="checkbox"/><label for="hz_resolution">'+lang['set26']+'</label>'+
	// Shortcut Tab
	...
	'<input id="hz_his" type="checkbox"/><label for="hz_his">'+lang['set20']+'</label><input id="hz_his_max" type="text" maxlength="4"/><label for="hz_his_columns">'+lang['set21']+'</label><input id="hz_his_columns" type="text" maxlength="1"/></div>'+
	'<div id="hz_set_save" class="button_style greenButton" title="'+lang['set02']+'">'+lang['set02']+'</div><div title="'+lang['set03']+'" class="button_style whiteButton" id="hz_set_clear">'+lang['set03'];
	...
	var main = function(){
		...
		// !Todo: 採用exec函數檢驗是否有重複記錄，若有，將其提到最前
		var history = function(){
			if ( url != $('#hz_history').children().eq(0).attr('href') ) {
				var time = new Date(),
		...
		var shortcut = function(){
			...
			$(sc).fadeIn(300).offset({'top': mouse.y+10, 'left': mouse.x+10}).bind({
				'mouseenter': function(){pause = 1;},
				'mouseleave': function(){pause = 0;}
			});
			var timerSC = function(){
				if (pause == 1){
					clearTimeout(timeout);
					setTimeout(timerSC, 1000);
				} else {
					clearTimeout(timeout);
		...
		var hide = function(){
			...
			pause = 0;
			clearTimeout(timer);
			$(hz).hide().html('<div class="loading"></div>');
		...
		var fullscreen = function(){
			...
			fs.onmousemove = ctrlbar;
			fs.childNodes[1].onmouseenter = function(){
				timeint = setInterval(function(){clearTimeout(timeout);}, 1000);
			};
			...
			fs.childNodes[1].onmouseleave = function(){clearInterval(timeint)};
		...
		clearTimeout(timer);
		if (options['hz_trigger'] == 0) timer = setTimeout(show, options['hz_delay']);
		if(options['hz_shortcut'] === 'true') shortcut();
		$(document).bind('keydown', function(e){
			var code = e.keyCode || e.which;
	...
	var history = function(){
		$('#hz_history_open').click(function(){
			...
			var	histories = localStorage['hz_histories'].split('|||'),
				...
				count = (his > options['hz_his_max']) ? options['hz_his_max'] : his-1,
			...
			if (count > 0){
				for (var i=0; i<count; i++){
					...
					appends += indiv[0] + ';' + indiv[1] + '|||';
				}
				$('#hz_history').append(prepends);
				localStorage['hz_histories'] = appends;
			}
			...
			$('#hz_history_list').click(function(){copyArea($('#hz_history'), count);});
			$('#hz_his_newtab').click(function(){newTab($('#hz_history'), count);});
		});
	...
	var albumDL = function(){
		...
		$.getJSON(aUrl, function(json){
			...
			if (typeof albumLink != 'undefined'){
				...
				$('#hz_albums_list').click(function(){copyArea($('#hz_albums'), albumCounts);});
				$('#hz_albums_newtab').click(function(){newTab($('#hz_albums'), albumCounts);});
			} else {
	...
	var allPic = function(){
		...
		$('#hz_allpic_list').click(function(){copyArea($('#hz_allpic'), count);});
		$('#hz_allpic_newtab').click(function(){newTab($('#hz_allpic'), count);});
	}
		...
		inComment: function(){
			...
			for (a=0; a<commLinks.length; a++){
				...
				if (options['hz_direct'] === 'true' && url.match(picRegex) && !$(item).hasClass('img-in-post')){
					...
					var maxWidth = (options['hz_direct_ytmaxwidth'] > 0) ? options['hz_direct_ytmaxwidth'] : item.parentNode.parentNode.offsetWidth,
							fragment = document.createDocumentFragment();
						url = url.replace(/(.*)watch\?v=(.*)/, '$1v/$2').replace(/&(.*)/g, '') + '?version=3&autohide=1&feature=player_embedded';
						var closeYT = document.createElement('div'),
							object = document.createElement('object');
						closeYT.className = 'closeYT';
						closeYT.title = lang['yt01'];
						closeYT.innerHTML = 'X';
						closeYT.onclick = function(){
							var parent = this.parentNode;
							this.previousSibling.removeAttribute('style');
							parent.removeChild(this.nextSibling);
							parent.removeChild(this);
						};
						object.style.height = maxWidth*aspect;
						object.style.width = maxWidth;
						object.innerHTML = '<param name="movie" value="'+url+'"><param name="allowFullScreen" value="true"><param name="allowScriptAccess" value="always"><embed src="'+url+'" type="application/x-shockwave-flash" allowfullscreen="true" allowScriptAccess="always" width="'+maxWidth+'" height="'+maxWidth*aspect+'">';
						fragment.appendChild(closeYT);
						fragment.appendChild(object);
					$(item).after(fragment);
					$(item).css({'display': 'block', 'fontWeight': 'bold', 'marginRight': 11}).addClass('yt-in-post');
				}
			}
		},
		album: function(){
			...
			if (pageUrl.match(/\/photos\/\w+\/albums\/\w+/)) {
				...
				$('.B-u-Y-j').each(function(){
					...
					if ( url.match(/albums/) && !$(this).hasClass('album-in-post') ) {
						...
						button.onclick = albumDL;
						$(this).parentsUntil('.Ve').find('.dl').append(button);

...
var batch = function(){
	...
	$('.uaGLLd div img, .ot-anchor').filter(':visible').each(function(){
		...
		$('.Vl .ot-anchor').each(function(){
			if (!$(this).data('class')){
				...
				if (url.match(picRegex)){
					...
					$(this).addClass('hz_img-in-post').css('width', width + 34).html('<img src="'+url+'" style="max-width:'+(options.hz_direct_post_max > 0 ? options.hz_direct_post_max + 'px' : '')+'"><div style="width:'+(width+32)+'px"></div>').parent().parent().css('paddingBottom', 0);
				}
			}

...
// Todo: 浮動下載快捷鍵、觸發鍵（熱鍵）、滑鼠移入大圖時不隱藏
		...
		var fullscreen = function(){
			...
			var insert = function(num, repeat){
				if (trigger == true){
					...
					var item = $('<img>').attr('src', url).load(function(){
						var img = this;
					...
					if (arr.length > 1){
						item.on('click', next).on('contextmenu', prev);
						$(fs).addClass('multi');
					}
				}
			}
			...
			$(fs).show().on('click', '.back, .close', close).on('click', '.prev', prev).on('click', '.next', next).on('scroll', function(){

...
var albumDL = function(){
	...
	var meta = function(length, author, album){
		var text;
		if (length > 1) {
			text = '<a href="'+author[1]+'">'+author[0]+'</a> &raquo; <a href="'+album[1]+'"><strong>'+album[0]+'</strong></a> ('+length+' '+lang.set08+')';
		} else {
			text = '<a href="'+author[1]+'">'+author[0]+'</a> &raquo; <a href="'+album[1]+'"><strong>'+album[0]+'</strong></a> ('+length+' '+lang.set07+')';
		}
	...
	GM_xmlhttpRequest({
		...
		onload: function(data){
			...
			meta(entry.length, [feed.author[0].name.$t, feed.author[0].uri.$t], [feed.title.$t, feed.link[1].href]);
			sortPic($page, fragment);
		}
	});
}

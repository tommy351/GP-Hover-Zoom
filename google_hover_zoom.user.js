	...
	links: function(){
		$('body').on('click', '.hz_stacks_downloadSelected', function(){
			$(this).parent().parent().find('input').filter(':checked').each(function(){
				openWindow($(this).prev().attr('href'));
			});
			...
			if (target.length == target.filter(':checked').length){
				...
				this.innerHTML = lang.piclink03;
			} else {
				target.prop('checked', true);
				this.innerHTML = lang.piclink04;
			}
		}).on('change', '.hz_stackItem input', function(){
			var target = $(this).parent().parent().find('input');
			if (target.length == target.filter(':checked').length){
				$(this).parent().parent().find('.hz_stacks_selectAll').html(lang.piclink04);
			} else {
				$(this).parent().parent().find('.hz_stacks_selectAll').html(lang.piclink03);
			}
		});
	},
	directDL: function(){
